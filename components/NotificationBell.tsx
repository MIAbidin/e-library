...
const typeColor: Record<string, string> = {
  new_book: 'bg-green-100',
  announcement: 'bg-blue-100',
  reading_reminder: 'bg-orange-100',
}
...
export function NotificationBell() {
  ...
  // Fetch HANYA notifikasi yang belum dibaca untuk dropdown
  const fetchUnreadNotifications = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/notifications?filter=unread&limit=10&page=1')
      const data = await res.json()
  ...
  const handleMarkAllRead = async () => {
    try {
      ...
      setUnreadCount(0)
      setNotifications([]) // Kosongkan karena sudah semua dibaca
    } catch {
      // Silent fail
    }
  }
        ...
        <div className="absolute right-0 top-full mt-2 w-80 sm:w-96
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
            <div className="flex items-center gap-2">
              <h3 className="font-semibold text-gray-900 text-sm">Belum Dibaca</h3>
              {unreadCount > 0 && (
          ...
          <div className="max-h-80 overflow-y-auto">
            {loading ? (
              ...
              <div className="text-center py-10">
                ...
                <p className="text-sm text-gray-500 font-medium">Semua sudah dibaca</p>
                <p className="text-xs text-gray-400 mt-1">Tidak ada notifikasi baru</p>
              </div>
            ) : (
              notifications.map((notif) => (
                <button
                  ...
                  className="w-full text-left px-4 py-3 hover:bg-gray-50
                    transition border-b border-gray-50 last:border-0 bg-blue-50/40"
                >
                  <div className="flex gap-3">
                    {/* Icon */}
                    <div className={`w-8 h-8 rounded-full flex items-center
                      justify-center text-base flex-shrink-0 mt-0.5
                      ${typeColor[notif.notification.type] ?? 'bg-gray-100'}`}>
